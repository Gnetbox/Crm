...
<script type="text/javascript">
	...
	function getActivity(){
		...
		//展现关联的市场活动列表
		$.ajax({
			url:"workbench/clue/activity.do",
			type:"post",
			dataType:"json",
			data:{"clueId":"${c.id}"},
			success:function (data){
				$.each(data,function (index,element){
					opt += '<tr id="'+element.id+'">';
					opt += '<td>'+element.name+'</td>';
					opt += '<td>'+element.startDate+'</td>';
					opt += '<td>'+element.endDate+'</td>';
					opt += '<td>'+element.owner+'</td>';
					opt += '<td><a href="javascript:void(0);"; onclick="del(\''+element.id+'\');" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					opt += '</tr>';
				})
